...
fig.tight_layout()
save_path = os.path.join(plots_dir, 'label_distribution.png')
plt.savefig(save_path)
plt.close()
...
# Add percentage labels to the bars
counts_type = df_non_frac['body_part'].value_counts()
total_non_frac = counts_type.sum()


for bar in bars.patches:
    value = int(bar.get_height())
    percentage = (value / total_non_frac) * 100
    ax.text(
        bar.get_x() + bar.get_width() / 2,
        value,
        f'{value} ({percentage:.1f}%)',
...
fig.tight_layout()
save_path = os.path.join(plots_dir, 'non_fractured_body_parts.png')
plt.savefig(save_path)
plt.close()
print(f"Saved plot to {save_path}")
...
for bar in bars.patches:
    value = int(bar.get_height())
    percentage = (value / total_frac) * 100
    ax.text(
        bar.get_x() + bar.get_width() / 2,
        value,
        f'{value} ({percentage:.1f}%)',
...
fig.tight_layout()
save_path = os.path.join(plots_dir, 'fractured_body_parts.png')
plt.savefig(save_path)
plt.close()
print(f"Saved plot to {save_path}")
...
def plot_bbox_count_distribution(frac_dir, annotation_dir, save_path=None):
    counts = []
...
plot_bbox_count_distribution(
    ...
    annotation_path,
    save_path=os.path.join(plots_dir, 'bbox_count_distribution.png')
)
...
def plot_selected_images_with_bboxes(frac_list, nonfrac_list, frac_dir, nonfrac_dir, annotation_dir, save_path=None):
...
plot_selected_images_with_bboxes(
    ...
    save_path=os.path.join(plots_dir, 'example_images_with_bboxes.png')
)

# Bounding box size distribution

def plot_bbox_size_distribution(frac_dir, annotation_dir, save_path=None):
    areas = []
    for filename in os.listdir(frac_dir):
        if filename.endswith('.jpg'):
...
plot_bbox_size_distribution(
    os.path.join(fracAtlas, 'images', 'Fractured'),
    annotation_path,
    save_path=os.path.join(plots_dir, 'bbox_size_distribution.png')
)
...
# Save the large boxes info to a file
with open(os.path.join(plots_dir, 'large_boxes_info.txt'), 'w') as f:
    f.write(f"Found {len(large_boxes_sorted)} bounding boxes with area > 10,000 px²:\n\n")
    for filename, area in large_boxes_sorted:
        f.write(f"{filename} — {area} px²\n")
print(f"Saved large boxes info to {os.path.join(plots_dir, 'large_boxes_info.txt')}")

## Checking the dimension of the images

def check_image_dimensions(image_dir, save_path=None):
    dimensions = {}
    ...
    for dim, count in dimensions.items():
        line = f"Size {dim[0]}x{dim[1]}: {count} images\n"
        result += line
        print(line.strip())
    ...
    if save_path:
        with open(save_path, 'w') as f:
            f.write(result)
        print(f"Saved dimensions info to {save_path}")


print("\nFractured:")
check_image_dimensions(
    os.path.join(fracAtlas, 'images', 'Fractured'),
    save_path=os.path.join(plots_dir, 'fractured_dimensions.txt')
)

print("\nNon Fractured:")
check_image_dimensions(
    os.path.join(fracAtlas, 'images', 'Non_fractured'),
    save_path=os.path.join(plots_dir, 'non_fractured_dimensions.txt')
)
...
def analyze_grayscale_intensity(frac_list, nonfrac_list, frac_dir, nonfrac_dir, bins=50, save_path=None):
    import numpy as np
    ...
    if save_path:
        ...
        with open(stats_path, 'w') as f:
            f.write(f"Mean pixel intensity (Fractured):     {mean_frac:.2f}\n")
            f.write(f"Mean pixel intensity (Not Fractured): {mean_nonfrac:.2f}\n")
        print(f"Saved intensity stats to {stats_path}")
...
analyze_grayscale_intensity(
    ...
    nonfrac_dir=os.path.join(fracAtlas, 'images', 'Non_Fractured'),
    bins=50,
    save_path=os.path.join(plots_dir, 'pixel_intensity_distribution.png')
)
...
nonfrac_embed, nonfrac_labels, nonfrac_files = extract_embeddings(nonfractured_full_list, non_fractured_path, "Non Fractured")

all_embeddings = frac_embed + nonfrac_embed
all_labels = frac_labels + nonfrac_labels
all_files = frac_files + nonfrac_files

plot_tsne(all_embeddings, all_labels, save_path=os.path.join(plots_dir, 'tsne_visualization.png'))
...
# Save the file counts to a log file
with open(os.path.join(plots_dir, 'file_counts.txt'), 'w') as f:
    f.write(f"{len(fractured_images)} fracture imgs sampled\n")
    f.write(f"{len(nonfractured_images)} non‑fracture imgs sampled\n")
    
    f.write("\nAll files in non‑fract folder:\n")
    for file in os.listdir(non_fractured_path):
        f.write(f"- {file}\n")
        
    f.write("\nFiltered .jpg only:\n")
    for file in [f for f in os.listdir(non_fractured_path) if f.lower().endswith('.jpg')]:
        f.write(f"- {file}\n")
print(f"Saved file counts to {os.path.join(plots_dir, 'file_counts.txt')}")
...
def plot_pca(embeddings, labels, save_path=None):
...
plot_pca(all_embeddings, all_labels, save_path=os.path.join(plots_dir, 'pca_visualization.png'))

# Detect outliers by distance from centroid
# Over / under exposed images

def detect_outliers_by_distance(embeddings, labels, filenames, threshold=1.5, save_path=None):
    embeddings = np.array(embeddings)
    outliers = []
    ...
    # Save outliers to file if path is provided
    if save_path:
        with open(save_path, 'w') as f:
            f.write("Outliers detected:\n")
            for fname, label, z in outliers:
                f.write(f"{fname} | Label: {label} | Z-score: {z:.2f}\n")
        print(f"Saved outliers to {save_path}")
    
    return outliers

outliers = detect_outliers_by_distance(
    all_embeddings, 
    all_labels, 
    all_files, 
    threshold=1.5,
    save_path=os.path.join(plots_dir, 'outliers.txt')
)

# Visual representation of outliers
def plot_outliers(embeddings, labels, filenames, outliers, save_path=None):
    # If we have outliers, create a PCA plot highlighting them
    if outliers:
        ...
        # Plot regular points first (non-outliers)
        for label in set(labels):
            # Indices of non-outliers with this label
            reg_idxs = [i for i, (fname, l) in enumerate(zip(filenames, labels)) 
                        if l == label and fname not in outlier_files]
            
            if reg_idxs:  # Only plot if we have non-outlier points
                plt.scatter(reduced[reg_idxs, 0], reduced[reg_idxs, 1], 
                            alpha=0.3, label=f"{label}", s=30)
        
        # Plot outliers with a different marker and color
        out_idxs = [i for i, fname in enumerate(filenames) if fname in outlier_files]
        if out_idxs:
            plt.scatter(reduced[out_idxs, 0], reduced[out_idxs, 1], 
        ...
        if save_path:
            plt.savefig(save_path)
            plt.close()
            print(f"Saved outliers plot to {save_path}")
        else:
            plt.show()

# Plot the outliers if we found any
if outliers:
    plot_outliers(
        all_embeddings, 
        all_labels, 
        all_files, 
        outliers,
        save_path=os.path.join(plots_dir, 'outliers_visualization.png')
    )

# We need the distribution from the meta data (how can this be used to put more light on the fractured images?)

# Additional visualization: Visualize the distribution of samples in the train/val/test splits

def plot_split_distributions(df_train, df_val, df_test, save_path=None):
    # Count the number of fractured/non-fractured samples in each set
    train_counts = df_train['label'].value_counts()
    val_counts = df_val['label'].value_counts()
    test_counts = df_test['label'].value_counts()
    
    # Make sure we have both classes in each set (0 and 1)
    ...
    # Extract counts for non-fractured (0) and fractured (1)
    non_fractured = [train_counts[0], val_counts[0], test_counts[0]]
    fractured = [train_counts[1], val_counts[1], test_counts[1]]
    ...
    rects1 = ax.bar(x - width/2, non_fractured, width, label='Non-fractured')
    rects2 = ax.bar(x + width/2, fractured, width, label='Fractured')
    
    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Number of Samples')
    ax.set_title('Distribution of Classes in Data Splits')
    ax.set_xticks(x)
    ...
    if save_path:
        plt.savefig(save_path)
        plt.close()
        print(f"Saved data split distribution to {save_path}")
    else:
        plt.show()
        
    # Also save the percentages in each split to a text file
    if save_path:
        stats_path = os.path.splitext(save_path)[0] + "_stats.txt"
        with open(stats_path, 'w') as f:
            ...
            f.write(f"Train Set: {total_train} samples\n")
            f.write(f"  - Non-fractured: {train_counts[0]} ({train_counts[0]/total_train*100:.1f}%)\n")
            f.write(f"  - Fractured: {train_counts[1]} ({train_counts[1]/total_train*100:.1f}%)\n\n")
            
            f.write(f"Validation Set: {total_val} samples\n")
            f.write(f"  - Non-fractured: {val_counts[0]} ({val_counts[0]/total_val*100:.1f}%)\n")
            f.write(f"  - Fractured: {val_counts[1]} ({val_counts[1]/total_val*100:.1f}%)\n\n")
            
            f.write(f"Test Set: {total_test} samples\n")
            f.write(f"  - Non-fractured: {test_counts[0]} ({test_counts[0]/total_test*100:.1f}%)\n")
            f.write(f"  - Fractured: {test_counts[1]} ({test_counts[1]/total_test*100:.1f}%)\n")
        
        print(f"Saved data split statistics to {stats_path}")

# Plot the distribution of samples in each split
plot_split_distributions(
    df_train, 
    df_val, 
    df_test,
    save_path=os.path.join(plots_dir, 'data_split_distribution.png')
)

# Create a summary file with all the analysis information
summary_path = os.path.join(plots_dir, 'analysis_summary.txt')
with open(summary_path, 'w') as f:
    f.write("# Exploratory Data Analysis Summary\n\n")
    
    f.write("## Dataset Overview\n")
    f.write(f"Total images: {len(df_meta)}\n")
    f.write(f"Fractured images: {df_meta['label'].sum()}\n")
    f.write(f"Non-fractured images: {len(df_meta) - df_meta['label'].sum()}\n\n")
    
    f.write("## Body Parts Distribution\n")
    f.write("Dominant body parts in fractured images:\n")
    body_parts_frac = df_frac['body_part'].value_counts()
    for part, count in body_parts_frac.items():
        f.write(f"- {part}: {count} ({count/len(df_frac)*100:.1f}%)\n")
    
    f.write("\nDominant body parts in non-fractured images:\n")
    body_parts_nonfrac = df_non_frac['body_part'].value_counts()
    for part, count in body_parts_nonfrac.items():
        f.write(f"- {part}: {count} ({count/len(df_non_frac)*100:.1f}%)\n\n")
    
    f.write("## Bounding Box Analysis\n")
    # Count number of annotation files
    annotation_count = len([f for f in os.listdir(annotation_path) if f.endswith('.txt')])
    f.write(f"Number of images with bounding box annotations: {annotation_count}\n\n")
    
    f.write("## Image Dimensions\n")
    f.write("See fractured_dimensions.txt and non_fractured_dimensions.txt for details.\n\n")
    
    f.write("## Pixel Intensity Analysis\n")
    f.write("See pixel_intensity_distribution.png and pixel_intensity_distribution_stats.txt for details.\n\n")
    
    f.write("## Embedding Visualizations\n")
    f.write("t-SNE and PCA visualizations created to explore feature space.\n")
    f.write("See tsne_visualization.png and pca_visualization.png.\n\n")
    
    f.write("## Outlier Analysis\n")
    f.write(f"Number of outliers detected: {len(outliers)}\n")
    f.write("See outliers.txt and outliers_visualization.png for details.\n\n")
    
    f.write("## Data Split Analysis\n")
    f.write(f"Train set: {len(df_train)} images\n")
    f.write(f"Validation set: {len(df_val)} images\n")
    f.write(f"Test set: {len(df_test)} images\n")
    f.write("See data_split_distribution.png and data_split_distribution_stats.txt for details.\n")

print(f"Created analysis summary at {summary_path}")

print("\nAll plots and analysis files have been saved to the 'plots' directory.")
