'''
Description: Infernce the trained faster R-CNN model on the test dataset.¨

Author: SEF, used with code from faster-RCNN repository
'''
...
class RCNNDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(im_info['detections']) > 0:
            ...
            targets['labels'] = torch.zeros((0,   ), dtype=torch.int64) # no fractures → zero objects - background class
...
def load_model_and_dataset(args):
    ...
    fracatlas = RCNNDataset('test', im_dir=dataset_config['im_test_path'], ann_dir=dataset_config['ann_test_path'])
    test_dataset = DataLoader(fracatlas, batch_size=1, shuffle=False)
    ...
    if args.use_resnet50_fpn:
        ...
        faster_rcnn_model.load_state_dict(torch.load(os.path.join(train_config['task_name'],
                                                     ...
                                                     map_location=device))
    return faster_rcnn_model, fracatlas, test_dataset


def infer(args):
    if args.use_resnet50_fpn:
        output_dir = '2. Train the models/faster R-CNN/fracatlas/samples_tv_r50fpn'
    else:
        output_dir = '2. Train the models/faster R-CNN/fracatlas/samples_tv'
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    faster_rcnn_model, fracatlas, test_dataset = load_model_and_dataset(args)

    for sample_count in tqdm(range(40)):
        random_idx = random.randint(0, len(fracatlas))
        im, target, fname = fracatlas[random_idx]
        ...
        for idx, box in enumerate(target['bboxes']):
            ...
            text = fracatlas.idx2label[target['labels'][idx].detach().cpu().item()]
            text_size, _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_PLAIN, 1, 1)
            text_w, text_h = text_size
            cv2.rectangle(gt_im_copy, (x1, y1), (x1 + 10 + text_w, y1 + 10 + text_h), [255, 255, 255], -1)
            cv2.putText(gt_im, text=fracatlas.idx2label[target['labels'][idx].detach().cpu().item()],
                        org=(x1 + 5, y1 + 15),
                        thickness=1,
                        fontScale=1,
                        color=[0, 0, 0],
        ...
        for idx, box in enumerate(boxes):
            ...
            text = '{} : {:.2f}'.format(fracatlas.idx2label[labels[idx].detach().cpu().item()],
...
def evaluate_map(args):
    ...
    for im, target, fname in tqdm(test_dataset):
        ...
        pred_boxes = {}
        gt_boxes = {}
        for label_name in fracatlas.label2idx:
        ...
        for idx, box in enumerate(boxes):
            ...
            label_name = fracatlas.idx2label[label]
    ...
    for idx in range(len(fracatlas.idx2label)):
        print('AP for class {} = {:.4f}'.format(fracatlas.idx2label[idx], all_aps[fracatlas.idx2label[idx]]))
