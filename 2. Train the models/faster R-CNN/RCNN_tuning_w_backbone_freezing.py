...
FREEZE_BACKBONE = True  # Flag to freeze backbone layers
...
def make_model(w_box, w_rpn, freeze_backbone=FREEZE_BACKBONE):
    ...
    model = fasterrcnn_resnet50_fpn_v2(
        ...
        box_nms_thresh=0.5,   # Adjusted NMS threshold
    ...
    if freeze_backbone:
        ...
        print(f"Backbone frozen successfully: {frozen_params}/{total_backbone_params} parameters frozen")
...
def objective(trial):
    ...
    # Use the freeze_backbone parameter when creating the model
    base = make_model(w_box, w_rpn, freeze_backbone=FREEZE_BACKBONE).to(DEVICE)
...
def run_inference(model, image_path, confidence_threshold=0.5):
    ...
    # This is a simplified version; full weighted box fusion would be more complex
    merged_boxes = []
    merged_scores = []
    merged_labels = []
...
def main():
    ...
    try:
        ...
        # Report backbone freezing status
        if FREEZE_BACKBONE:
            print("Model will be trained with frozen backbone")
        else:
            print("Full model including backbone will be trained")
        ...
        for trial in sorted(trial_data, key=lambda x: x['Loss']):
            # Handle the case where weight_decay might be 'N/A' (a string)
            weight_decay_val = trial['Weight Decay']
            if isinstance(weight_decay_val, str):
                weight_decay_str = weight_decay_val
            else:
                weight_decay_str = f"{weight_decay_val:^10.6f}"
                
            print(f"{trial['Trial']:^5} | {trial['Loss']:^10.6f} | {trial['Learning Rate']:^10.6f} | "
                f"{weight_decay_str} | {trial['Cls Weight']:^8.2f} | {trial['Box Weight']:^8.2f} | "
        ...
        if FINAL_EPOCHS > 0:
            ...
            # Use the freeze_backbone parameter when creating the model
            base = make_model(best['w_box'], best['w_rpn'], freeze_backbone=FREEZE_BACKBONE).to(DEVICE)
            ...
            scaler = GradScaler()
            # Use AdamW with the optimal weight decay, but only optimize trainable parameters
            opt = torch.optim.AdamW(
                filter(lambda p: p.requires_grad, base.parameters()),
                lr=best['lr'], 
